#PBS -N HTMC_CTL_seurat_ct_to_matrixEQTL
...
CTDIR=$OUTDIR/HTMC_CTL_${CT}
mkdir -p $CTDIR

# CTL
echo "Starting CTL file preparation"
# This script will have run prior (only runs to create object with all)
# COUNTDIR=$OUTDIR/HTMC_count_array_v015
# Rscript $DIR/main/v015/HTMC_aggr_seurat.R "${COUNTDIR}/HTMC_pool-?_GRCh38/*_cellbender/*.h5" $AGGOBJ
# Aggregate CellRanger Counts output
# Aggregated Seurat object
AGGOBJ=$OUTDIR/HTMC_CTL_merged.RDS
...
IDXPATH=$CTDIR/HTMC_CTL_${CT}_index.csv

# The path for PEER matrix (per donor sums of log1p, z-scaled SCT counts)
PMXPATH=$CTDIR/HTMC_CTL_${CT}_QC11drsum_mtx.csv
PCVPATH=$CTDIR/HTMC_CTL_${CT}_peer_covariates.csv
FACPATH=$CTDIR/HTMC_CTL_${CT}_peer_factors.csv
RESPATH=$CTDIR/HTMC_CTL_${CT}_peer_residuals.csv

# The path for MatrixEQTL matrix (per donor means of SCT counts)
MMXPATH=$CTDIR/HTMC_CTL_${CT}_SCTdrmean_mtx.csv
# The path for MatrixEQTL covariates (will get PFs added)
MCVPATH=$CTDIR/HTMC_CTL_${CT}_matrixEQTL_covariates.csv
SNPPATH=$CTDIR/HTMC_CTL_${CT}_SNPs.csv
QTLPATH=$CTDIR/HTMC_CTL_${CT}_eQTLs
...
# MatrixEQTL
echo "Starting CTL MatrixEQTL file preparation"
